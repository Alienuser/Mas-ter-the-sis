...
Zum einen gibt es das System des \textit{Watson Studios}. In diesem wird das neuronale Netz trainiert. Nach erfolgreichem
training wird es als Modul in ein Deployment installiert. Dieses Deployment enthält eine REST-Schnittstelle, damit Anfragen
an das Model getätigt werden können.
...
Nun ist der Inhalt der Datei sichtbar. Bei dieser Ansicht ist darauf zu achten, dass die Werte der einzelnen Spalten als
...
Baustein ermöglicht den Import der konvertierten Trainingsdaten für das neuronale Netz. Über Drag\&Drop kann der Nutzer
den Baustein in den noch leeren Arbeitsbereich platzieren.

Um den Baustein zu konfigurieren klickt der Entwickler doppelt auf den Baustein. Dies öffnet einen rechten seitlichen
Arbeitsbereich. In diesem kann er über die Schaltfläche \texttt{Change Data Asset} die Datei auswählen, welche die
Trainingssätzen enthält. In dieser Liste steht lediglich die CSV-Datei zur Verfügung. Über \texttt{Save} wird die
Einstellung gespeichert und der Baustein ist fertig konfiguriert.

Im nächsten Schritt muss man das neuronale Netz konfigurieren. Dazu existiert in der Kategorie \textit{Modeling} das
Modul \textit{Neural Net}. Dies stellt für die Anwendung die beste Alternative dar. Genau wie das Import-Modul kann
...
die jeweilige Kategorie. Dabei beschreiben die Targets die Variablen, welche durch den Watson Service Vorhergesagt werden
...
Unter dem ersten Menüpunkt, Model Information, kann man alle generellen Informationen finden.
...
Weiter ist hier zu finden, mit wie vielen Inputparameter (englisch Number of Predictors) das Netz trainiert ist und wie
...
die Leistung erhöht werden), muss das Band mit höherer Geschwindigkeit (Bandgeschwindigkeit) drehen um die Produkte zu
transportieren.
...
Im Eingabe-Parameter \textit{values} müssen ebenfalls die Output-Parameter definiert werden. Da die Werte keine Rolle
spielen, werden die Werte mit \textit{null} initialisiert. Damit werden sie vom System nicht weiter verwendet.
...
Über den Menüpunkt \texttt{Run Yout API}, gelangt man auf eine weitere Anleitung, welche das starten und zur Verfügung
